...
class MakerLoop:
        ...
        try:
            while state != LoopState.DONE:
                ...
                if state == LoopState.FARMING:
                    ...
                    # Reconcile fills
                    await self._reconcile_fills(market_id, active_orders, summary)
    ...
    async def _farm_orders(
        ...
        if summary.down_shares < self.farm_max_shares:
            if (now - last_down_post_at) >= FARM_REFRESH_S:
                await self._post_ladder(
    ...
    async def _reconcile_fills(
        ...
        if not self.dry_run:
            try:
                live_orders = await asyncio.to_thread(self.bot.get_open_orders, market_id)
